...
from TSForecasting.layers.Patch_layers import *
from TSForecasting.layers.RevIN import RevIN
...
class ConvEncoder(nn.Module):
    def __init__(self, kernel_size, d_model, d_ff=None,
        ...
        self.layers = nn.ModuleList([ConvEncoderLayer(d_model, d_ff=d_ff, kernel_size=kernel_size[i], dropout=dropout,
                                                      ...
                                                      norm=norm,
                                                      re_param=re_param, small_ks=re_param_kernel, device=device) for i
                                     in range(n_layers)])
...
class ConvEncoderLayer(nn.Module):
    def __init__(self, d_model: int, d_ff: int = 256, kernel_size: int = 9, dropout: float = 0.1,
                 activation: str = "gelu", enable_res_param=True, norm='batch', re_param=True, small_ks=3,
                 device='cuda:0'):
